...
def lorenzSysNR(X, C, tauAB, tauAC, xa, ya, xb, yb, xc, yc):
	...
	#equations for fval
	eq1 = math.sqrt((x - xb)**2 + (x - yb)**2) - math.sqrt((x - xa)**2 + (x - ya)**2) - C*tauAB
	eq2 = math.sqrt((x - xc)**2 + (x - yc)**2) - math.sqrt((x - xa)**2 + (x - ya)**2) - C*tauAC
...
def main():
	X = multiVarNR(-0.10, -0.0866, 0.010, -0.0866, 0.0, 0.0866, 0.000023, 0.000039, 0.000045)
	print(X)
